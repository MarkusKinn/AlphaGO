...
DEFAULT_AUTH = os.getenv('IT3105_AUTH')
...
class ActorClient:
    ...
    def handle(self, msg):
        ...
        if topic == 'message':
            ...
            answer = self.handle_question(
                question=msg['question'], prompt=msg['prompt']
            )
            self.send(answer)
        ...
        # === Hooks for player logic
            ...
            self.handle_game_over(
                winner=msg['winner'], end_state=msg['end_state'],
            )
    ...
    def get_random_action(self, state):
        ...
        return random.choice([
            ...
            for index, cell in enumerate(state[1:]) if cell == 0
        ])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--host', help='Server host hostess', default=DEFAULT_HOST
    )
    parser.add_argument(
        '--api-port', help='Server API host port', default=DEFAULT_API_PORT
    )
    parser.add_argument(
        '--cert', help='Server ssl certificate', default=DEFAULT_CERT
    )
    parser.add_argument(
        '--echo', help='Log debug', action='store_true', default=DEFAULT_ECHO
    )

    subparsers = parser.add_subparsers(help='Commands')
    subparsers.required = True

    parser_rrt = subparsers.add_parser(
        'request-reset-token', help='Request token reset email'
    )
    ...
    parser_rt.add_argument(
        '--temp-token', help='Temporary token received from email'
    )
    ...
    parser_gp.add_argument(
        '--auth', help='API authentication token', default=DEFAULT_AUTH
    )
    ...
    parser_uh.add_argument(
        '--auth', help='API authentication token', default=DEFAULT_AUTH
    )
    parser_uh.add_argument(
        '--handle', help='New handle ("" for random)'
    )

    args = vars(parser.parse_args())
    client = ActorClient(**{
        key: args.pop(key, None) for key in
        ['host', 'api_port', 'cert', 'auth', 'echo']
    })
    try:
